              ...
              >
                ...
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <div className="w-14 h-14 rounded-2xl bg-brand-primary text-neutral-white flex items-center justify-center shadow-md group-hover:scale-105 transition-transform">
                      <Icon className="w-7 h-7" />
                    </div>
                    <span className="text-xs font-semibold text-brand-medium bg-brand-light/10 px-3 py-1 rounded-full">
                      {service.subtitle}
                    </span>
                  </div>

                  <h3 className="text-xl sm:text-2xl font-bold text-brand-primary mb-3">
                    {service.title}
                  </h3>

                  <ul className="space-y-2.5 mb-6">
                    {service.highlights.map((item, hIdx) => (
                      <li key={hIdx} className="flex items-start text-sm text-charcoal-primary">
                        <span className="w-1.5 h-1.5 rounded-full bg-brand-secondary mt-2 mr-2.5 shrink-0" />
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="pt-4 border-t border-neutral-light/60">
                  <a
                    href={service.href}
                    className="inline-flex items-center justify-center min-h-[48px] px-5 py-2.5 rounded-xl text-sm font-bold text-neutral-white bg-brand-primary hover:bg-brand-secondary transition-all w-full sm:w-auto shadow-md group-hover:shadow-lg"
                  >
